...
// 56025 進口：實驗線六項都是 Math.round 的整數（55351–55396），實驗線走得到的狀態換順序逐位元相同；這裡驗的是公式層（本線函式收任意數，big 給兩位小數）。
// 不存的（可證逐位元相同，存了只會是等價突變）：①左結合加總頭兩項互換（roadUpkeep+parks*.5 → parks*.5+roadUpkeep、55990 消防組↔警察組）：
// IEEE 加法可交換，第一個部分和相同，後面每一步都相同；②55990 組內、55974／55975 各項換順序：整數座數乘整數或 .5 的倍數，部分和都是 .5 的倍數、
// 遠小於 2^52，每一步都精確，怎麼排都一樣（55976 有 ×1.2、×.8，55973 第一項 roadUpkeep 帶尾數，都不在此列）
