...
export interface CityHit { id: number; x: number; z: number }
...
export function buildCityScene(c: City, look: KindLook, style: Style): BuiltCity {
  ...
  for (const a of owners.values()) for (const id of a) if (id) drawn.add(id);
  mark('buildings');
